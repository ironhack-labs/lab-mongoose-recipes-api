...
app.get("/recipes", (req, res) => {
  Recipe.find()
    .then((allRecipes) => {
      console.log("Retrieved recipe ->", allRecipes);
      res.status(200).json(allRecipes);
    })
    .catch((error) => {
      console.error("Error while retrieving allrecipe ->", error);
      res.status(500).json({ error: "Failed to retrieve allrecipe" });
    });
});
