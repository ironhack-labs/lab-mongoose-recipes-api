...
app.post("/recipes", (req, res, next) => {
    ...
    .catch((error) => {
        res.status(500).json({ message: "Error when creating recipe" });
    });
    })
...
app.get("/recipes/:id", (req, res, next) => {
    ...
    Recipe.findById(req.params.id)
        ...
        .catch((error) => {
            ...
            res.status(500).json({ message: "Error when getting single recipe" });
        });
});
...
app.put("/recipes/:id", (req, res, next) => {
    ...
    Recipe.findByIdAndUpdate(req.params.id, req.body, { new: true })
        ...
        .catch((error) => {
            res.status(500).json({ message: "Error when updating single recipe" });
        });
});
